  ...
  for (var i = 0; i < s.length; i++) {
    ...
    if (isNormalCharacter && this.isEsc == 0 /* ESnormal */) {
      ...
      if (!this.printing) {
        ...
        if (this.insertMode) {
          // avoid scrolling if not necessary
          if(this.cursorY < this.console[this.currentScreen].childNodes.length && this.console[this.currentScreen].childNodes[this.cursorY].textContent.replace(/\s+$/,"").length > this.cursorX) {
          this.scrollRegion(this.cursorX, this.cursorY,
